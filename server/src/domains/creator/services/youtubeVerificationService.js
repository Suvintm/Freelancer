...
export const initiateVerification = async (userId, channelInput, metadata = {}) => {
  ...
  const accountCount = await prisma.youTubeChannel.count({ where: { userId } });
  if (accountCount >= 5) {
    throw new ApiError(
      403,
      "You have reached the maximum limit of 5 linked YouTube accounts. Please delete an existing channel to link a new one."
    );
  }
  ...
  if (genCount > 3) {
    throw new ApiError(
      429,
      "You have reached the daily limit (3) for channel verification requests. Please try again tomorrow."
    );
  }
  ...
  const verifyData = {
    ...
    identifier,
    type,
    niche: niche || metadata.subCategoryId || metadata.category || null,
    language,
    attemptCount: 0,
    createdAt: Date.now(),
  };
...
export const verifyChannel = async (userId, channelInput, reqData) => {
  ...
  if (state.attemptCount > 5) {
    ...
    throw new ApiError(
      429,
      "Too many failed attempts. Verification session locked. Please generate a new code."
    );
  }
  ...
  if (!channelData.description.includes(token)) {
    throw new ApiError(
      422,
      "Verification token not found in channel description. Please ensure you saved the changes on YouTube."
    );
  }
  ...
  if (incomingHash !== state.tokenHash) {
    logger.error(
      `🚨 [SECURITY] Token mismatch detected for User ${userId} on Channel ${channelData.channelId}`
    );
  ...
  if (existingChannel) {
    if (existingChannel.userId === userId) {
      ...
      throw new ApiError(
        409,
        "This channel is already claimed by another user and cannot be linked."
      );
    }
  }
  ...
  const newProfile = await prisma.youTubeChannel.create({
    data: {
