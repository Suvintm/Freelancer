    ...
    try {
      ...
      if (channels.length > 0) {
        ...
        if (process.env.YT_SYNC_MODE === "foreground" || process.env.YT_SYNC_MODE === "manual") {
          logger.info(
            `✨ [CREATOR-EVENTS] Found ${channels.length} channels for new user ${userId}, but skipping BullMQ because YT_SYNC_MODE=${process.env.YT_SYNC_MODE}. Sync will be triggered manually by the frontend.`
          );
        } else {
          logger.info(
            `✨ [CREATOR-EVENTS] Found ${channels.length} channels for new user ${userId}, scheduling sync in BullMQ.`
          );
