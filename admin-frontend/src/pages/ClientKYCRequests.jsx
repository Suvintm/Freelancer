...
const ClientKYCRequests = () => {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await adminAxios.get("/client-kyc/admin/stats");
      if (res.data.success) {
  ...
  const fetchKYC = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminAxios.get("/client-kyc/admin/pending", {
        params: { status: statusFilter, page: pagination.page, limit: 20 }
      });
      if (res.data.success) {
        setUsers(res.data.kycList || []);
  ...
  const approveKYC = async (id) => {
    ...
    try {
      const res = await adminAxios.post(`/client-kyc/admin/verify/${id}`, { action: "approve" });
      if (res.data.success) {
        toast.success("Client KYC approved successfully!");
  ...
  const rejectKYC = async () => {
    ...
    setActionLoading(true);
    try {
      const res = await adminAxios.post(`/client-kyc/admin/verify/${selectedUser._id}`, {
        action: "reject",
        rejectionReason: rejectReason,
      });
      if (res.data.success) {
        toast.success("Client KYC rejected");
        setSelectedUser(null);
  ...
  // Filter users by search (client-side for now if backend search not implemented)
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-zinc-400 text-sm mt-1">Verify client identities for refunds and security</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-white">{stats.not_started || 0}</p>
        ...
        <div className="overflow-x-auto">
          ...
          <thead>
            <tr className="border-b border-zinc-800">
              <th className="text-left text-xs font-medium text-zinc-400 uppercase px-4 py-3">Client</th>
              <th className="text-left text-xs font-medium text-zinc-400 uppercase px-4 py-3 hidden md:table-cell">Account Holder</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="text-center py-12 text-zinc-500">
                  No client KYC submissions found
                </td>
              </tr>
            ) : (
              filteredUsers.map(record => (
                <tr key={record._id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <img
                        src={record.user?.profilePicture || "https://cdn-icons-png.flaticon.com/512/3135/3135715.png"}
                        alt={record.user?.name}
                        className="w-10 h-10 rounded-full object-cover"
                      />
                      <div>
                        <p className="text-white font-medium text-sm">{record.user?.name || "Unknown"}</p>
                        <p className="text-zinc-500 text-xs">{record.user?.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <div>
                      <p className="text-zinc-300 text-sm">{record.bankDetails?.accountHolderName || "-"}</p>
                      <p className="text-zinc-500 text-xs">{record.bankDetails?.bankName}</p>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-center gap-2">
                       {/* Pass record._id which is the KYC document ID */}
                      <button
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {[
                    { label: "Account Holder", value: selectedUser.accountHolderName },
                    { label: "Bank Name", value: selectedUser.bankName },
                    { label: "Account Number", value: selectedUser.bankAccountNumber },
                   ...
                   {selectedUser.upiId && (
                    <div className="bg-zinc-800 rounded-lg p-3 col-span-2">
                      <p className="text-[10px] text-zinc-500 uppercase tracking-wide">UPI ID</p>
                      <p className="text-white text-sm font-medium">{selectedUser.upiId}</p>
                    </div>
                  )}
                </div>
              ...
              {selectedUser.status === "pending" && (
                <div className="p-5 border-t border-zinc-800 flex gap-3">
                  ...
                  <button
                    onClick={() => approveKYC(selectedUser._id)}
            ...
            >
              ...
              <div className="p-5">
                <textarea
                  ...
                  placeholder="e.g., Invalid bank document..."
                  rows={4}
...
export default ClientKYCRequests;
