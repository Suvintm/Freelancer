...
const Orders = () => {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      const res = await adminAxios.patch(`/admin/orders/${orderId}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  const exportToCSV = () => {
    const headers = ["Order #", "Title", "Client", "Editor", "Status", "Amount", "Date"];
  ...
  const ShimmerRow = () => (
    <tr className="animate-pulse">
      ...
      <td className="p-4"><div className="h-4 w-28 shimmer rounded" /></td>
      <td className="p-4"><div className="h-6 w-20 shimmer rounded-full" /></td>
      <td className="p-4"><div className="h-4 w-16 shimmer rounded" /></td>
      <td className="p-4"><div className="h-8 w-16 shimmer rounded" /></td>
    </tr>
  );
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedOrder(null)}>
          <motion.div
            ...
            className="bg-dark-700 border border-dark-500 rounded-2xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto"
          >
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between items-center text-sm p-3 bg-dark-600 rounded-lg">
                <span className="text-gray-400">Status</span>
                <select
                  value={selectedOrder.status}
                  onChange={(e) => handleStatusChange(selectedOrder._id, e.target.value)}
                  className="bg-dark-700 border border-dark-500 rounded-lg px-3 py-1 text-sm focus:outline-none focus:border-purple-500"
                >
                  {Object.entries(STATUS_CONFIG).map(([key, val]) => (
                    <option key={key} value={key}>{val.label}</option>
                  ))}
            ...
            <button onClick={() => setSelectedOrder(null)} className="w-full py-3 bg-dark-600 rounded-xl text-gray-400 hover:text-white transition-colors">Close</button>
          </motion.div>
        </div>
      )}
    </div>
